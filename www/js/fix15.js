...
const OLD_MODAL_START = 'window.__app.showIslandResourcesModal = () => {';
const OLD_MODAL_END_PATTERN = "  showModal('🌿 Recursos da Ilha', modalHtml);\n};\n";

if (ui.includes(OLD_MODAL_START)) {
  const startIndex = ui.indexOf(OLD_MODAL_START);
  const endIndex = ui.indexOf(OLD_MODAL_END_PATTERN, startIndex) + OLD_MODAL_END_PATTERN.length;
  
  const NEW_MODAL = `window.__app.showIslandResourcesModal = () => {
  ...
  showModal('🌿 Recursos da Ilha (Detalhado)', modalHtml);
};
`;

  ui = ui.substring(0, startIndex) + NEW_MODAL + ui.substring(endIndex);
  // Bump version for cache busting
  ui = ui.replace(/v=93/g, 'v=94');
  
  fs.writeFileSync('js/ui.js', ui);
  console.log('ui.js updated with new modal');
  
  // also bump html version just in case
