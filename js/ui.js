...
function renderMarketFiltered(search = '', filter = 'inventory') {
  ...
  // ── FEED filter: live market activity & daily summary ──
  if (filter === 'feed') {
    renderMarketFeed(search);
    return;
  }

  // ── ORDERBOOK filter: item depth & bids/offers ──
  if (filter === 'orderbook') {
    renderMarketOrderBook(search);
    return;
  }

  // ── AUCTIONS filter: live drops & winner leaderboard ──
  if (filter === 'auctions') {
    renderMarketAuctions(search);
    return;
  }

  // ── PROFILE filter: commercial profile of any farm ──
  if (filter === 'profile') {
    renderMarketProfile(search);
    return;
  }

  // ── CATALOG filter: master tradeables catalog ──
  if (filter === 'catalog') {
    renderMarketCatalog(search);
    return;
  }
...
// =====================================================
// COMMUNITY API VIEWS (FEED, ORDERBOOK, AUCTIONS, PROFILE, CATALOG)
// =====================================================

let _currentOrderBookItem = { collection: 'collectibles', id: 601, name: 'Victoria Sisters' };
let _searchedFarmId = null;
let _catalogCategory = 'all';

function openOrderBook(collection, id, encodedName) {
  _currentOrderBookItem = {
    collection: collection || 'collectibles',
    id: id || 601,
    name: decodeURIComponent(encodedName || 'Victoria Sisters')
  };
  const chips = document.querySelectorAll('.filter-chips[data-target="market"] .chip');
  chips.forEach(c => {
    if (c.dataset.filter === 'orderbook') c.classList.add('active');
    else c.classList.remove('active');
  });
  const activeHolder = document.getElementById('market-filter-active');
  if (activeHolder) activeHolder.dataset.filter = 'orderbook';
  renderMarketFiltered('', 'orderbook');
}

async function renderMarketFeed(search = '', forceRefresh = false) {
  setHtml('#market-grid', `
    <div style="grid-column:1/-1; text-align:center; padding:40px 20px;">
      <div class="spinner" style="margin:0 auto 12px;"></div>
      <div style="font-size:13px; color:var(--text-tertiary);">Carregando feed de atividade da API...</div>
    </div>
  `);
  ...
  let data = null;
  try {
    data = await API.getMarketplaceActivity(forceRefresh);
  } catch(e) {
    console.warn('[Feed error]', e);
  }

  const reports = data?.reports || {};
  const latestDate = Object.keys(reports).sort().reverse()[0];
  const report = reports[latestDate] || {};
  const totals = report.totals || data?.totals || { volume: 0, trades: 0 };
  const rawItems = report.items || data?.items || {};
  const flowerPrice = data?.flowerPrice || 0.1345;
  const isFallback = data?.isFallback ?? true;

  const totalUsd = totals.volume * flowerPrice;

  let itemsList = Object.entries(rawItems).map(([itemKey, stats]) => {
    let name = stats.name || itemKey;
    if (name.includes('-')) {
    ...
    return {
      key: itemKey,
      name,
      collection: stats.collection || (itemKey.startsWith('wearables') ? 'wearables' : 'collectibles'),
      id: stats.id || parseInt(itemKey.split('-')[1]) || 0,
      low: stats.low ?? 0,
      high: stats.high ?? 0,
      volume: stats.volume ?? 0,
      trades: stats.trades ?? 0,
      quantity: stats.quantity ?? 0,
      latestSale: stats.latestSale ?? stats.floor ?? 0,
      floor: stats.floor ?? 0,
      listingCount: stats.listingCount ?? 0,
      offerCount: stats.offerCount ?? 0,
      bestOffer: stats.bestOffer ?? 0
    };
  });

  if (search.trim()) {
    const q = search.toLowerCase();
    itemsList = itemsList.filter(i => i.name.toLowerCase().includes(q) || i.key.toLowerCase().includes(q));
  }

  itemsList.sort((a, b) => b.volume - a.volume);

  const statusBadge = isFallback
    ? `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(245,158,11,0.15); color:var(--amber); font-weight:700; border:1px solid rgba(245,158,11,0.3);">🟡 Demonstração (Sem API Key)</span>`
    : `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(16,185,129,0.15); color:var(--emerald); font-weight:700; border:1px solid rgba(16,185,129,0.3);">🟢 Community API Oficial</span>`;
  ...
  const html = `
    <div style="grid-column:1/-1; display:flex; flex-direction:column; gap:14px; margin-bottom:8px;">
      <div style="background:var(--surface-2); border:1px solid var(--surface-border); border-radius:16px; padding:16px 18px;">
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
          <div>
            <div style="display:flex; align-items:center; gap:8px;">
              <span style="font-size:14px; font-weight:800; color:var(--text-primary);">⚡ Atividade & Feed de Mercado</span>
              ${statusBadge}
            </div>
            <div style="font-size:11px; color:var(--text-tertiary); margin-top:2px;">
              Volume e negociações em tempo real reportados pela rede
            </div>
          </div>
          <button onclick="window.__app.UI.renderMarketFeed('${search}', true)" style="background:var(--surface-3); border:1px solid var(--surface-border); color:var(--text-primary); border-radius:10px; padding:6px 12px; font-size:12px; font-weight:700; cursor:pointer; display:flex; align-items:center; gap:6px;">
            🔄 Atualizar
          </button>
        </div>

        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(130px, 1fr)); gap:12px; margin-top:14px; padding-top:14px; border-top:1px solid var(--surface-border);">
          <div>
            <div style="font-size:10px; color:var(--text-tertiary); text-transform:uppercase; font-weight:700;">Volume 24h</div>
            <div style="font-size:17px; font-weight:900; color:var(--emerald); margin-top:2px;">
              ${totals.volume.toLocaleString('pt-BR', { maximumFractionDigits: 1 })} <span style="font-size:11px; font-weight:700;">FLOWER</span>
            </div>
            <div style="font-size:11px; color:var(--text-tertiary);">≈ $${totalUsd.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD</div>
          </div>
          <div>
            <div style="font-size:10px; color:var(--text-tertiary); text-transform:uppercase; font-weight:700;">Negociações</div>
            <div style="font-size:17px; font-weight:900; color:var(--sky); margin-top:2px;">
              ${totals.trades.toLocaleString('pt-BR')} <span style="font-size:11px; font-weight:700;">trades</span>
            </div>
            <div style="font-size:11px; color:var(--text-tertiary);">Movimentações hoje</div>
          </div>
          <div>
            <div style="font-size:10px; color:var(--text-tertiary); text-transform:uppercase; font-weight:700;">Preço FLOWER</div>
            <div style="font-size:17px; font-weight:900; color:var(--amber); margin-top:2px;">
              $${flowerPrice.toFixed(4)} <span style="font-size:11px; font-weight:700;">USD</span>
            </div>
            <div style="font-size:11px; color:var(--text-tertiary);">Cotação de referência</div>
          </div>
        </div>
      </div>
    </div>
    ...
    ${itemsList.length === 0 ? `
      <div class="empty-state" style="grid-column:1/-1">
        <span class="empty-state-icon">🔍</span>
        <div class="empty-state-title">Nenhum item encontrado no feed</div>
      </div>
    ` : itemsList.map(item => `
      <div class="market-card" style="display:flex; flex-direction:column; gap:10px; padding:14px; background:var(--surface-2); border:1px solid var(--surface-border); border-radius:14px;">
        <div style="display:flex; justify-content:space-between; align-items:flex-start;">
          <div style="display:flex; align-items:center; gap:10px;">
            <div style="width:40px; height:40px; background:var(--surface-3); border-radius:10px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
              <img src="${window.getImgUrl(item.name)}" style="width:28px; height:28px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
            </div>
            <div>
              <div style="font-size:14px; font-weight:800; color:var(--text-primary);">${item.name}</div>
              <div style="font-size:11px; color:var(--text-tertiary); text-transform:capitalize;">${item.collection} · #${item.id}</div>
            </div>
          </div>
          <button onclick="window.__app.UI.openOrderBook('${item.collection}', ${item.id}, '${encodeURIComponent(item.name)}')" style="background:var(--emerald-subtle); border:1px solid var(--emerald); color:var(--emerald); border-radius:8px; padding:4px 10px; font-size:11px; font-weight:700; cursor:pointer;">
            📖 Livro
          </button>
        </div>

        <div style="display:grid; grid-template-columns:1fr 1fr; gap:6px; padding:8px 0; border-top:1px solid var(--surface-border); border-bottom:1px solid var(--surface-border);">
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Última Venda:</div>
            <div style="font-size:13px; font-weight:800; color:var(--text-primary);">${item.latestSale > 0 ? item.latestSale.toFixed(4) + ' SFL' : '—'}</div>
          </div>
          <div style="text-align:right;">
            <div style="font-size:10px; color:var(--text-tertiary);">Piso (Floor):</div>
            <div style="font-size:13px; font-weight:800; color:var(--emerald);">${item.floor > 0 ? item.floor.toFixed(4) + ' SFL' : '—'}</div>
          </div>
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Faixa (Min-Max):</div>
            <div style="font-size:11px; font-weight:700; color:var(--text-secondary);">${item.low.toFixed(4)} - ${item.high.toFixed(4)}</div>
          </div>
          <div style="text-align:right;">
            <div style="font-size:10px; color:var(--text-tertiary);">Volume 24h:</div>
            <div style="font-size:11px; font-weight:700; color:var(--sky);">${item.volume.toLocaleString('pt-BR', { maximumFractionDigits: 1 })} SFL (${item.trades} trades)</div>
          </div>
        </div>

        <div style="display:flex; justify-content:space-between; align-items:center; font-size:11px; color:var(--text-tertiary);">
          <span>Ordens: <b style="color:var(--text-secondary);">${item.listingCount} vendas</b> / <b style="color:var(--text-secondary);">${item.offerCount} compras</b></span>
          ${item.bestOffer > 0 ? `<span>Melhor Oferta: <b style="color:var(--amber);">${item.bestOffer.toFixed(4)} SFL</b></span>` : ''}
        </div>
      </div>
    `).join('')}
  `;
...
async function renderMarketOrderBook(search = '') {
  const item = _currentOrderBookItem;
  setHtml('#market-grid', `
    <div style="grid-column:1/-1; text-align:center; padding:40px 20px;">
      <div class="spinner" style="margin:0 auto 12px;"></div>
      <div style="font-size:13px; color:var(--text-tertiary);">Carregando livro de ofertas de ${item.name}...</div>
    </div>
  `);
  ...
  let data = null;
  try {
    data = await API.getMarketplaceItem(item.id, item.collection);
  } catch(e) {
    console.warn('[OrderBook error]', e);
  }
  ...
  const quickItems = [
    { name: 'Victoria Sisters', col: 'collectibles', id: 601 },
    { name: 'Immortal Pear', col: 'collectibles', id: 415 },
    { name: 'Golden Cauliflower', col: 'collectibles', id: 702 },
    { name: 'Sunflower Shield', col: 'wearables', id: 112 },
    { name: 'Rocket Onesie', col: 'wearables', id: 250 },
    { name: 'Heart Air Balloon', col: 'collectibles', id: 805 },
  ];
  ...
  const html = `
    <div style="grid-column:1/-1; display:flex; flex-direction:column; gap:14px; margin-bottom:8px;">
      <div style="display:flex; gap:6px; overflow-x:auto; padding-bottom:6px; scrollbar-width:none;">
        ${quickItems.map(qi => `
          <button onclick="window.__app.UI.openOrderBook('${qi.col}', ${qi.id}, '${encodeURIComponent(qi.name)}')" style="white-space:nowrap; padding:6px 12px; border-radius:20px; font-size:11px; font-weight:700; cursor:pointer; background:${qi.id === item.id ? 'var(--emerald)' : 'var(--surface-2)'}; color:${qi.id === item.id ? 'var(--obsidian-base)' : 'var(--text-secondary)'}; border:1px solid ${qi.id === item.id ? 'var(--emerald)' : 'var(--surface-border)'};">
            ${qi.name}
          </button>
        `).join('')}
      </div>
      ...
      <div style="background:var(--surface-2); border:1px solid var(--surface-border); border-radius:16px; padding:16px 18px;">
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
          <div style="display:flex; align-items:center; gap:12px;">
            <div style="width:48px; height:48px; background:var(--surface-3); border-radius:12px; display:flex; align-items:center; justify-content:center;">
              <img src="${window.getImgUrl(item.name)}" style="width:34px; height:34px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
            </div>
            <div>
              <div style="font-size:16px; font-weight:900; color:var(--text-primary);">${item.name}</div>
              <div style="font-size:11px; color:var(--text-tertiary); text-transform:capitalize;">${item.collection} · ID #${item.id}</div>
            </div>
          </div>
          <div>
            ${isFallback ? `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(245,158,11,0.15); color:var(--amber); font-weight:700; border:1px solid rgba(245,158,11,0.3);">🟡 Demonstração</span>` : `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(16,185,129,0.15); color:var(--emerald); font-weight:700; border:1px solid rgba(16,185,129,0.3);">🟢 Livro Oficial</span>`}
          </div>
        </div>

        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(110px, 1fr)); gap:10px; margin-top:14px; padding-top:14px; border-top:1px solid var(--surface-border);">
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Piso (Floor):</div>
            <div style="font-size:15px; font-weight:900; color:var(--emerald);">${floorPrice > 0 ? floorPrice.toFixed(4) + ' SFL' : '—'}</div>
          </div>
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Última Venda:</div>
            <div style="font-size:15px; font-weight:900; color:var(--text-primary);">${lastSalePrice > 0 ? lastSalePrice.toFixed(4) + ' SFL' : '—'}</div>
          </div>
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Suprimento:</div>
            <div style="font-size:15px; font-weight:900; color:var(--sky);">${supply.toLocaleString('pt-BR')} un.</div>
          </div>
        </div>
      </div>
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:12px;">
        <div style="background:var(--surface-2); border:1px solid var(--surface-border); border-radius:14px; padding:14px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
            <span style="font-size:13px; font-weight:800; color:var(--emerald);">🟢 Vendedores (Listings)</span>
            <span style="font-size:11px; color:var(--text-tertiary);">${listings.length} ofertas</span>
          </div>
          ${listings.length === 0 ? `
            <div style="font-size:12px; color:var(--text-tertiary); text-align:center; padding:16px;">Sem vendedores ativos no momento.</div>
          ` : `
            <div style="display:flex; flex-direction:column; gap:6px; max-height:300px; overflow-y:auto; padding-right:4px;">
              ${listings.map(l => `
                <div style="display:flex; justify-content:space-between; align-items:center; padding:6px 10px; background:var(--surface-3); border-radius:8px; font-size:12px;">
                  <div>
                    <span style="font-weight:700; color:var(--text-primary);">Fazenda #${l.farmId || l.farm || '?'}</span>
                    <div style="font-size:10px; color:var(--text-tertiary);">Qtd: ${l.quantity}</div>
                  </div>
                  <div style="text-align:right;">
                    <div style="font-weight:900; color:var(--emerald);">${(l.sfl || 0).toFixed(4)} SFL</div>
                    <div style="font-size:10px; color:var(--text-tertiary);">Total: ${((l.sfl || 0) * (l.quantity || 1)).toFixed(2)} SFL</div>
                  </div>
                </div>
              `).join('')}
            </div>
        ...
        <div style="background:var(--surface-2); border:1px solid var(--surface-border); border-radius:14px; padding:14px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
            <span style="font-size:13px; font-weight:800; color:var(--sky);">🔵 Compradores (Offers)</span>
            <span style="font-size:11px; color:var(--text-tertiary);">${offers.length} propostas</span>
          </div>
          ${offers.length === 0 ? `
            <div style="font-size:12px; color:var(--text-tertiary); text-align:center; padding:16px;">Sem compradores ativos no momento.</div>
          ` : `
            <div style="display:flex; flex-direction:column; gap:6px; max-height:300px; overflow-y:auto; padding-right:4px;">
              ${offers.map(o => `
                <div style="display:flex; justify-content:space-between; align-items:center; padding:6px 10px; background:var(--surface-3); border-radius:8px; font-size:12px;">
                  <div>
                    <span style="font-weight:700; color:var(--text-primary);">Fazenda #${o.farmId || o.farm || '?'}</span>
                    <div style="font-size:10px; color:var(--text-tertiary);">Qtd: ${o.quantity}</div>
                  </div>
                  <div style="text-align:right;">
                    <div style="font-weight:900; color:var(--sky);">${(o.sfl || 0).toFixed(4)} SFL</div>
                    <div style="font-size:10px; color:var(--text-tertiary);">Total: ${((o.sfl || 0) * (o.quantity || 1)).toFixed(2)} SFL</div>
                  </div>
                </div>
              `).join('')}
            </div>
...
async function renderMarketAuctions(search = '') {
  setHtml('#market-grid', `
    <div style="grid-column:1/-1; text-align:center; padding:40px 20px;">
      <div class="spinner" style="margin:0 auto 12px;"></div>
      <div style="font-size:13px; color:var(--text-tertiary);">Carregando leilões e resultados da API...</div>
    </div>
  `);
  ...
  if (search.trim()) {
    const q = search.toLowerCase();
    filtered = filtered.filter(a => (a.wearable || a.nft || a.auctionId || '').toLowerCase().includes(q));
  }

  const html = `
    <div style="grid-column:1/-1; display:flex; flex-direction:column; gap:14px; margin-bottom:8px;">
      <div style="background:var(--surface-2); border:1px solid var(--surface-border); border-radius:16px; padding:16px 18px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
        <div>
          <div style="display:flex; align-items:center; gap:8px;">
            <span style="font-size:15px; font-weight:800; color:var(--text-primary);">🔨 Leilões do Auctioneer</span>
            ${isFallback ? `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(245,158,11,0.15); color:var(--amber); font-weight:700;">🟡 Demonstração</span>` : `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(16,185,129,0.15); color:var(--emerald); font-weight:700;">🟢 API Oficial</span>`}
          </div>
          <div style="font-size:11px; color:var(--text-tertiary); margin-top:2px;">Drops exclusivos de Wearables e NFTs com histórico de lances vencedores</div>
        </div>
      </div>
    </div>
    ...
    ${filtered.length === 0 ? `
      <div class="empty-state" style="grid-column:1/-1">
        <span class="empty-state-icon">🔨</span>
        <div class="empty-state-title">Nenhum leilão encontrado</div>
      </div>
    ` : filtered.map(auc => {
      const itemName = auc.wearable || auc.nft || 'Item';
      const isLive = now >= auc.startAt && now <= auc.endAt;
      const isEnded = now > auc.endAt;
      const isUpcoming = now < auc.startAt;

      let statusBadge = '';
      if (isLive) statusBadge = '<span style="color:var(--emerald); font-weight:800; font-size:11px; padding:2px 8px; border-radius:6px; background:rgba(16,185,129,0.15);">🟢 AO VIVO</span>';
      else if (isUpcoming) statusBadge = '<span style="color:var(--sky); font-weight:800; font-size:11px; padding:2px 8px; border-radius:6px; background:rgba(14,165,233,0.15);">⏳ EM BREVE</span>';
      else statusBadge = '<span style="color:var(--text-tertiary); font-weight:700; font-size:11px; padding:2px 8px; border-radius:6px; background:rgba(255,255,255,0.06);">FINALIZADO</span>';

      const ingredientsList = Object.entries(auc.ingredients || {}).map(([ing, qty]) => `${qty} ${ing}`).join(' + ');

      return `
        <div class="market-card" style="display:flex; flex-direction:column; gap:10px; padding:14px; background:var(--surface-2); border:1px solid var(--surface-border); border-radius:14px;">
          <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            <div style="display:flex; align-items:center; gap:10px;">
              <div style="width:44px; height:44px; background:var(--surface-3); border-radius:10px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                <img src="${window.getImgUrl(itemName)}" style="width:30px; height:30px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
              </div>
              <div>
                <div style="font-size:14px; font-weight:800; color:var(--text-primary);">${itemName}</div>
                <div style="font-size:11px; color:var(--text-tertiary); text-transform:uppercase;">${auc.type} · Suprimento: <b>${auc.supply} un.</b></div>
              </div>
            </div>
            ${statusBadge}
          </div>

          <div style="padding:8px 0; border-top:1px solid var(--surface-border); border-bottom:1px solid var(--surface-border); font-size:12px; display:flex; flex-direction:column; gap:4px;">
            <div style="display:flex; justify-content:space-between;">
              <span style="color:var(--text-tertiary);">Custo de Entrada:</span>
              <span style="font-weight:800; color:var(--amber);">${auc.sfl} SFL ${ingredientsList ? '+ ' + ingredientsList : ''}</span>
            </div>
            <div style="display:flex; justify-content:space-between;">
              <span style="color:var(--text-tertiary);">Data:</span>
              <span style="color:var(--text-secondary);">${new Date(auc.startAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</span>
            </div>
          </div>

          <div style="display:flex; justify-content:flex-end;">
            <button onclick="window.__app.UI.openAuctionResults('${auc.auctionId}', '${encodeURIComponent(itemName)}')" style="background:var(--emerald-subtle); border:1px solid var(--emerald); color:var(--emerald); border-radius:8px; padding:6px 12px; font-size:11px; font-weight:700; cursor:pointer; display:flex; align-items:center; gap:6px;">
              🏆 Ver Vencedores e Lances
            </button>
          </div>
        </div>
      `;
...
async function openAuctionResults(auctionId, encodedName) {
  const itemName = decodeURIComponent(encodedName);
  showModal(`🏆 Vencedores: ${itemName}`, `<div style="text-align:center; padding:20px;"><div class="spinner"></div><div style="margin-top:8px; font-size:12px; color:var(--text-tertiary);">Consultando resultados da API...</div></div>`);

  const API = (window.__app && window.__app.API) || {};
  let res = null;
  try {
    res = await API.getAuctionResults(auctionId);
  } catch(e) {
    console.warn(e);
  }
  ...
  const participants = res?.participantCount || leaderboard.length;

  const modalHtml = `
    <div style="display:flex; flex-direction:column; gap:12px;">
      <div style="display:flex; justify-content:space-between; align-items:center; padding-bottom:10px; border-bottom:1px solid var(--surface-border);">
        <span style="font-size:12px; color:var(--text-secondary);">Status: <strong style="color:var(--emerald); text-transform:uppercase;">${status}</strong></span>
        <span style="font-size:12px; color:var(--text-secondary);">Participantes: <strong style="color:white;">${participants}</strong></span>
      </div>

      <div style="max-height:400px; overflow-y:auto; padding-right:4px; display:flex; flex-direction:column; gap:6px;">
        ${leaderboard.length === 0 ? `
          <div style="text-align:center; padding:20px; color:var(--text-tertiary); font-size:12px;">Ainda não há lances registrados ou leilão em andamento.</div>
        ` : leaderboard.map(l => {
          const rankColor = l.rank === 1 ? '#FFD700' : (l.rank === 2 ? '#C0C0C0' : (l.rank === 3 ? '#CD7F32' : 'var(--text-tertiary)'));
          const rankIcon = l.rank === 1 ? '🥇' : (l.rank === 2 ? '🥈' : (l.rank === 3 ? '🥉' : `#${l.rank}`));
          const itemsStr = Object.entries(l.items || {}).map(([it, q]) => `${q} ${it}`).join(', ');

          return `
            <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 12px; background:var(--surface-2); border-radius:10px; border:1px solid var(--surface-border); font-size:12px;">
              <div style="display:flex; align-items:center; gap:10px;">
                <span style="font-weight:900; color:${rankColor}; font-size:14px; min-width:24px;">${rankIcon}</span>
                <div>
                  <div style="font-weight:800; color:var(--text-primary);">${l.username || `Fazenda #${l.farmId}`}</div>
                  <div style="font-size:10px; color:var(--text-tertiary);">Farm #${l.farmId} · XP: ${(l.experience || 0).toLocaleString()}</div>
                </div>
              </div>
              <div style="text-align:right;">
                <div style="font-weight:900; color:var(--emerald);">${(l.sfl || 0)} SFL</div>
                ${itemsStr ? `<div style="font-size:10px; color:var(--amber); font-weight:700;">+ ${itemsStr}</div>` : ''}
              </div>
            </div>
          `;
        }).join('')}
  ...
  showModal(`🏆 Vencedores: ${itemName}`, modalHtml);
}

async function renderMarketProfile(search = '', targetFarmId = null) {
  const farmId = targetFarmId || _searchedFarmId || Storage.getSettings()?.farmId || 2601876;
  _searchedFarmId = farmId;

  setHtml('#market-grid', `
    <div style="grid-column:1/-1; text-align:center; padding:40px 20px;">
      <div class="spinner" style="margin:0 auto 12px;"></div>
      <div style="font-size:13px; color:var(--text-tertiary);">Carregando perfil comercial da fazenda #${farmId}...</div>
    </div>
  `);
  ...
  let profile = null;
  try {
    profile = await API.getMarketplaceProfile(farmId);
  } catch(e) {
    console.warn('[Profile error]', e);
  }

  const isFallback = profile?.isFallback ?? true;
  const username = profile?.username || `Fazenda #${farmId}`;
  const totalTrades = profile?.totalTrades || 0;
  const profit = profile?.profit || 0;
  const weeklySpent = profile?.weeklyFlowerSpent || 0;
  const weeklyEarned = profile?.weeklyFlowerEarned || 0;
  const listings = Array.isArray(profile?.listings) ? profile.listings : Object.values(profile?.listings || {});
  ...
  const html = `
    <div style="grid-column:1/-1; display:flex; flex-direction:column; gap:14px; margin-bottom:8px;">
      <div style="display:flex; gap:8px;">
        <input type="number" id="input-profile-farm-id" value="${farmId}" placeholder="Digitar Farm ID..." style="flex:1; padding:10px 14px; background:var(--surface-2); border:1px solid var(--surface-border); border-radius:10px; color:var(--text-primary); font-family:var(--font-mono); font-size:13px;">
        <button onclick="window.__app.UI.renderMarketProfile('', document.getElementById('input-profile-farm-id').value)" style="background:var(--emerald); color:var(--obsidian-base); border:none; border-radius:10px; padding:0 16px; font-weight:800; font-size:12px; cursor:pointer;">
          🔍 Buscar
        </button>
      </div>
      ...
      <div style="background:var(--surface-2); border:1px solid var(--surface-border); border-radius:16px; padding:16px 18px;">
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
          <div style="display:flex; align-items:center; gap:12px;">
            <div style="width:48px; height:48px; background:var(--surface-3); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:24px;">
              👨‍🌾
            </div>
            <div>
              <div style="font-size:16px; font-weight:900; color:var(--text-primary);">${username}</div>
              <div style="font-size:11px; color:var(--text-tertiary);">Farm ID: <b>#${farmId}</b> · Nível: <b>${profile?.level || 1}</b></div>
            </div>
          </div>
          <div>
            ${isFallback ? `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(245,158,11,0.15); color:var(--amber); font-weight:700;">🟡 Demonstração</span>` : `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(16,185,129,0.15); color:var(--emerald); font-weight:700;">🟢 Perfil Oficial</span>`}
          </div>
        </div>

        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(110px, 1fr)); gap:10px; margin-top:14px; padding-top:14px; border-top:1px solid var(--surface-border);">
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Total Negociações:</div>
            <div style="font-size:16px; font-weight:900; color:var(--sky);">${totalTrades.toLocaleString('pt-BR')} trades</div>
          </div>
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Lucro Líquido:</div>
            <div style="font-size:16px; font-weight:900; color:${profit >= 0 ? 'var(--emerald)' : 'var(--coral)'};">${profit >= 0 ? '+' : ''}${profit.toLocaleString('pt-BR', { maximumFractionDigits: 1 })} SFL</div>
          </div>
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Gasto Semanal:</div>
            <div style="font-size:15px; font-weight:800; color:var(--coral);">${weeklySpent.toFixed(2)} SFL</div>
          </div>
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Ganho Semanal:</div>
            <div style="font-size:15px; font-weight:800; color:var(--emerald);">${weeklyEarned.toFixed(2)} SFL</div>
          </div>
        </div>
      </div>

      <div style="background:var(--surface-2); border:1px solid var(--surface-border); border-radius:14px; padding:14px;">
        <div style="font-size:13px; font-weight:800; color:var(--text-primary); margin-bottom:10px;">📦 Anúncios Ativos no Mercado (${listings.length})</div>
        ${listings.length === 0 ? `
          <div style="font-size:12px; color:var(--text-tertiary); text-align:center; padding:12px;">Nenhum anúncio ativo no momento.</div>
        ` : `
          <div style="display:flex; flex-direction:column; gap:6px;">
            ${listings.map(l => {
              const itName = l.item || (l.items ? Object.keys(l.items)[0] : 'Item');
              const itQty = l.quantity || (l.items ? Object.values(l.items)[0] : 1);
              return `
                <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 12px; background:var(--surface-3); border-radius:10px; font-size:12px;">
                  <div style="display:flex; align-items:center; gap:8px;">
                    <img src="${window.getImgUrl(itName)}" style="width:24px; height:24px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
                    <div>
                      <span style="font-weight:800; color:var(--text-primary);">${itName}</span>
                      <span style="font-size:10px; color:var(--text-tertiary);"> x ${itQty}</span>
                    </div>
                  </div>
                  <div style="font-weight:900; color:var(--emerald);">${(l.sfl || 0).toFixed(4)} SFL</div>
                </div>
      ...
      <div style="background:var(--surface-2); border:1px solid var(--surface-border); border-radius:14px; padding:14px;">
        <div style="font-size:13px; font-weight:800; color:var(--text-primary); margin-bottom:10px;">🤝 Últimas Negociações Realizadas (${trades.length})</div>
        ${trades.length === 0 ? `
          <div style="font-size:12px; color:var(--text-tertiary); text-align:center; padding:12px;">Sem histórico recente de negociações.</div>
        ` : `
          <div style="display:flex; flex-direction:column; gap:6px; max-height:280px; overflow-y:auto; padding-right:4px;">
            ${trades.map(t => {
              const itName = t.item || (t.itemId ? `Item #${t.itemId}` : 'Item');
              const isSell = t.type === 'sell';
              return `
                <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 12px; background:var(--surface-3); border-radius:10px; font-size:12px;">
                  <div style="display:flex; align-items:center; gap:8px;">
                    <span style="font-size:11px; padding:2px 6px; border-radius:4px; font-weight:800; background:${isSell ? 'rgba(16,185,129,0.15)' : 'rgba(14,165,233,0.15)'}; color:${isSell ? 'var(--emerald)' : 'var(--sky)'};">
                      ${isSell ? 'VENDA' : 'COMPRA'}
                    </span>
                    <span style="font-weight:700; color:var(--text-primary);">${itName} (x${t.quantity || 1})</span>
                  </div>
                  <div style="font-weight:900; color:${isSell ? 'var(--emerald)' : 'var(--text-secondary)'};">
                    ${isSell ? '+' : ''}${(t.sfl || 0).toFixed(4)} SFL
                  </div>
                </div>
              `;
            }).join('')}
      ...
      ${friends.length > 0 ? `
        <div style="background:var(--surface-2); border:1px solid var(--surface-border); border-radius:14px; padding:14px;">
          <div style="font-size:13px; font-weight:800; color:var(--text-primary); margin-bottom:10px;">👥 Principais Parceiros Comerciais</div>
          <div style="display:flex; flex-wrap:wrap; gap:8px;">
            ${friends.map(f => `
              <div onclick="window.__app.UI.renderMarketProfile('', ${f.id})" style="padding:6px 12px; background:var(--surface-3); border:1px solid var(--surface-border); border-radius:20px; font-size:11px; cursor:pointer; display:flex; align-items:center; gap:6px;">
                <span style="font-weight:800; color:var(--text-primary);">${f.username || `Fazenda #${f.id}`}</span>
                <span style="color:var(--emerald); font-weight:700;">${f.trades} trades</span>
              </div>
            `).join('')}
          </div>
        </div>
...
async function renderMarketCatalog(search = '', categoryFilter = null) {
  if (categoryFilter) _catalogCategory = categoryFilter;

  setHtml('#market-grid', `
    <div style="grid-column:1/-1; text-align:center; padding:40px 20px;">
      <div class="spinner" style="margin:0 auto 12px;"></div>
      <div style="font-size:13px; color:var(--text-tertiary);">Carregando catálogo mestre da API...</div>
    </div>
  `);
  ...
  let data = null;
  try {
    data = await API.getTradeableCatalog();
  } catch(e) {
  ...
  if (_catalogCategory !== 'all') {
    filtered = filtered.filter(i => (i.collection || i.type) === _catalogCategory);
  }
  if (search.trim()) {
    const q = search.toLowerCase();
    filtered = filtered.filter(i => (i.name || '').toLowerCase().includes(q) || String(i.id).includes(q));
  }

  const cats = [
    { id: 'all', label: 'Todos' },
    { id: 'collectibles', label: 'Colecionáveis' },
    { id: 'wearables', label: 'Roupas & Wearables' },
  ];
  ...
  const html = `
    <div style="grid-column:1/-1; display:flex; flex-direction:column; gap:12px; margin-bottom:8px;">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
        <div style="display:flex; align-items:center; gap:8px;">
          <span style="font-size:15px; font-weight:800; color:var(--text-primary);">📚 Catálogo de Itens Negociáveis</span>
          ${isFallback ? `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(245,158,11,0.15); color:var(--amber); font-weight:700;">🟡 Demonstração</span>` : `<span style="font-size:11px; padding:3px 8px; border-radius:12px; background:rgba(16,185,129,0.15); color:var(--emerald); font-weight:700;">🟢 Catálogo Oficial</span>`}
        </div>
      </div>

      <div style="display:flex; gap:6px; overflow-x:auto; padding-bottom:4px;">
        ${cats.map(c => `
          <button onclick="window.__app.UI.renderMarketCatalog('${search}', '${c.id}')" style="white-space:nowrap; padding:6px 14px; border-radius:20px; font-size:11px; font-weight:700; cursor:pointer; background:${_catalogCategory === c.id ? 'var(--emerald)' : 'var(--surface-2)'}; color:${_catalogCategory === c.id ? 'var(--obsidian-base)' : 'var(--text-secondary)'}; border:1px solid ${_catalogCategory === c.id ? 'var(--emerald)' : 'var(--surface-border)'};">
            ${c.label}
          </button>
        `).join('')}
      </div>
    ...
    ${filtered.length === 0 ? `
      <div class="empty-state" style="grid-column:1/-1">
        <span class="empty-state-icon">📚</span>
        <div class="empty-state-title">Nenhum item encontrado no catálogo</div>
      </div>
    ` : filtered.map(item => `
      <div class="market-card" style="display:flex; flex-direction:column; gap:8px; padding:12px; background:var(--surface-2); border:1px solid var(--surface-border); border-radius:14px;">
        <div style="display:flex; align-items:center; gap:10px;">
          <div style="width:40px; height:40px; background:var(--surface-3); border-radius:10px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
            <img src="${window.getImgUrl(item.name)}" style="width:28px; height:28px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
          </div>
          <div style="flex:1; min-width:0;">
            <div style="font-size:13px; font-weight:800; color:var(--text-primary); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${item.name}</div>
            <div style="font-size:10px; color:var(--text-tertiary); text-transform:capitalize;">${item.collection || item.type} · ID #${item.id}</div>
          </div>
        </div>

        <div style="display:flex; justify-content:space-between; align-items:center; padding-top:6px; border-top:1px solid var(--surface-border); font-size:11px;">
          <div>
            <div style="font-size:10px; color:var(--text-tertiary);">Piso (Floor):</div>
            <div style="font-weight:900; color:var(--emerald);">${item.floor ? item.floor.toFixed(4) + ' SFL' : '—'}</div>
          </div>
          <button onclick="window.__app.UI.openOrderBook('${item.collection || 'collectibles'}', ${item.id}, '${encodeURIComponent(item.name)}')" style="background:var(--emerald-subtle); border:1px solid var(--emerald); color:var(--emerald); border-radius:6px; padding:4px 8px; font-size:10px; font-weight:700; cursor:pointer;">
            📖 Livro
          </button>
        </div>
      </div>
...
function renderAlertsPage() {
  ...
  const html = `
    ...
    <div class="card mb-3" style="padding:14px 16px; background:var(--surface-2); border:1px solid var(--surface-border); display:flex; justify-content:space-between; align-items:center;">
      <div>
        <div style="font-size:13px; font-weight:800; color:var(--text-primary);">Testar Notificações</div>
        <div style="font-size:11px; color:var(--text-tertiary);">Dispara um alerta de teste com imagem pixelada oficial</div>
      </div>
      <button onclick="window.__app.Notifications && window.__app.Notifications.testNotification()" style="background:var(--emerald-subtle); border:1px solid var(--emerald); color:var(--emerald); border-radius:10px; padding:8px 14px; font-size:12px; font-weight:700; cursor:pointer;">
        🧪 Testar Alerta
      </button>
    </div>
    ...
    <div class="card" style="padding:0; overflow:hidden;">
      ...
      <div style="padding:8px 0; max-height: 50vh; overflow-y: auto;">
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Sunflower')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Chicken')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Apple')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Wood')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Stone')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Honey')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Red Balloon Flower')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Oil Reserve')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
            <span style="font-size:13px; font-weight:700;">Poços de Óleo</span>
          </div>
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Basic Compost')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
            <span style="font-size:13px; font-weight:700;">Composteiras</span>
          </div>
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Rice')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Mashed Potato')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
            <span style="font-size:13px; font-weight:700;">Construções (Cozinhas)</span>
          </div>
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Crab')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Sun')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
            <span style="font-size:13px; font-weight:700;">Santuários</span>
          </div>
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Salt')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Delivery')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
        ...
        <div style="padding:12px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.02);">
          <div style="display:flex; align-items:center; gap:10px;">
            <img src="${window.getImgUrl('Heart Air Balloon')}" style="width:22px; height:22px; object-fit:contain; image-rendering:pixelated;" onerror="this.style.display='none'">
...
window.__app.UI.openOrderBook = openOrderBook;
window.__app.UI.renderMarketFeed = renderMarketFeed;
window.__app.UI.renderMarketOrderBook = renderMarketOrderBook;
window.__app.UI.renderMarketAuctions = renderMarketAuctions;
    ...
    const renderResults = (query) => {
      ...
      document.querySelectorAll('.wallet-search-item').forEach(el => {
        el.addEventListener('click', () => {
          ...
          document.getElementById('wallet-selected-img').src = imgUrl(selectedItemName);
