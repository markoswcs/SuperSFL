...
class NotificationEngine {
      ...
      if (prevListings && prevListings.length > 0) {
        for (const prev of prevListings) {
          ...
          if (!stillExists) {
            ...
            if (itemName && sfl > 0) {
              const img = window.getImgUrl ? window.getImgUrl(itemName) : `https://sfl.world/img/source/${encodeURIComponent(itemName)}.png`;
              await this.sendPush(`${itemName} vendido!`, {
  ...
  async testNotification() {
    const img = window.getImgUrl ? window.getImgUrl('Sunflower') : 'https://sfl.world/img/source/Sunflower.png';
    const sent = await this.sendPush('Girassol pronto para colheita!', {
      body: 'Sua plantação de girassol está pronta para colher no jogo.',
      icon: img,
      largeIcon: img,
    });
    if (window.__app?.UI?.showToast) {
      window.__app.UI.showToast('Notificação de teste disparada com imagem pixelada!', 'success', img);
    }
    return sent;
  }
}
